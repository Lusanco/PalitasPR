<script>
  ...
  /* // Ensure valid length for month (2 digits)
    if (input.id === "month" && value.length >= 2 && /[0-9]/.test(key)) {
      event.preventDefault();
      return;
    }

    // Ensure valid length for day (2 digits)
    if (input.id === "day" && value.length >= 2 && /[0-9]/.test(key)) {
      event.preventDefault();
      return;
    }

    // Ensure valid length for year (4 digits)
    if (input.id === "year" && value.length >= 4 && /[0-9]/.test(key)) {
      event.preventDefault();
      return;
    }

    // Ensure valid range for month (01-12)
    if (input.id === "month" && /[0-9]/.test(key)) {
      const newValue = value + key;
      if (newValue.length === 2 && (newValue < "01" || newValue > "12")) {
        event.preventDefault();
        return;
      }
    }

    // Ensure valid range for day (01-31)
    if (input.id === "day" && /[0-9]/.test(key)) {
      const newValue = value + key;
      if (newValue.length === 2 && (newValue < "01" || newValue > "31")) {
        event.preventDefault();
        return;
      }
    }
        ...
        >
          ...
          <div>
            <p class="text-xs md:text-sm">
              Describa el servicio ofrecido. Puede añadir más de un artículo a
              la lista.
            </p>
            <!--* Details input -->
