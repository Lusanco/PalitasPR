...
>
    ...
    <div class="grid grid-cols-2 grid-rows-2 rounded-lg bg-base-100 overflow-clip">
      <div id="search-bar" class="col-span-2 row-span-1">
        ...
        <input
          ...
          class="form-input w-full col-span-2 rounded-t-md border-base-200 focus:border-primary py-2.5 pe-10 shadow-sm sm:text-sm"
        />
        <!-- on:keydown={handleKeydown} -->
      </div>
      <div id="filters" class="grid grid-cols-3 col-span-2 row-span-1">
        ...
        <select
          ...
          class="block w-full overflow-y-auto border-0 border-b-2 form-select border-neutral-200 focus:border-primary focus:ring-0 disabled:cursor-not-allowed"
        >
  ...
  {:else if (!$state.hidden && !$state.loaded) || $state.reload}
    <Loading />
  {:else if $state.error}
    <span class="font-bold text-primary">No results found, try again.</span>
  {:else}
    <div
      class="flex py-2 flex-col gap-4 rounded-md w-[95%] sm:w-[90%] md:w-[80%] overflow-y-scroll overflow-hidden h-96 bg-base-100"
    >
      {#each services as service}
        <a href="##">
          <div
            class="relative block w-full p-4 border rounded-lg shadow-lg border-base-100 sm:p-6 lg:p-8"
          >
            <span
            ...
            <div class="sm:flex sm:justify-between sm:gap-4">
              <div>
                ...
                <p class="mt-1 text-xs font-medium text-neutral">
            ...
            <dl class="flex gap-4 sm:gap-6">
              <div class="flex flex-col-reverse">
                <dt class="text-sm font-medium text-neutral">Published</dt>
                <dd class="text-xs text-neutral">{service.created_at}</dd>
              </div>
