...
<div class="max-h-full min-h-screen mt-6">
  <h1 class="font-sans text-3xl text-center text-primary text-bold">Services</h1>
  <br />
  <br />
  <div class="h-screen overflow-scroll overscroll-y-auto">
    {#each offers as offer}
      <div class="flex flex-row items-start w-full p-4 mb-4 border shadow-lg">
        <!-- svelte-ignore a11y-img-redundant-alt -->
        <img
          src={offer.profilePicture}
          alt="Profile Picture"
          class="object-cover h-20 mr-4 rounded-full w-18"
        />
        <div class="flex-1">
          <div class="font-bold text-primary">{offer.name}</div>
          <div>{offer.priceRange}</div>
          <div>ID: {offer.id}</div>
